var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _a, _b;
import express from 'express';
import urlValidationZod from './zod/urlValidationZod.js';
import { createClient } from '@libsql/client';
import dotenv from 'dotenv';
import cors from 'cors';
var app = express();
dotenv.config();
var PORT = (_a = process.env.PORT) !== null && _a !== void 0 ? _a : 3000;
var db = createClient({
    url: (_b = process.env.DB_URL) !== null && _b !== void 0 ? _b : "",
    authToken: process.env.DB_TOKEN
});
await db.execute("\n    CREATE TABLE IF NOT EXISTS \"url\" (\n      \"id\"\tTEXT NOT NULL,\n      \"original_url\"\tTEXT NOT NULL,\n      PRIMARY KEY(\"id\")\n    );\n  ");
app.use(express.json());
app.use(cors());
app.post('/create', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var url, _a, data, success, UID, idURL, id, e_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                url = req.body.url;
                _a = urlValidationZod(url), data = _a.data, success = _a.success;
                if (!success) return [3 /*break*/, 6];
                _b.label = 1;
            case 1:
                _b.trys.push([1, 4, , 5]);
                UID = 'IU' + Math.floor(Math.random() * 999999);
                return [4 /*yield*/, db.execute({
                        sql: "INSERT INTO url (id, original_url) VALUES ($UID, $data)",
                        args: { UID: UID, data: data }
                    })];
            case 2:
                _b.sent();
                return [4 /*yield*/, db.execute({
                        sql: "SELECT id FROM url WHERE id = $UID",
                        args: { UID: UID }
                    })];
            case 3:
                idURL = _b.sent();
                id = idURL.rows[0];
                res.status(201).json(id);
                return [3 /*break*/, 5];
            case 4:
                e_1 = _b.sent();
                res.status(500).json({ message: 'Something went wrong :,(' });
                return [3 /*break*/, 5];
            case 5: return [3 /*break*/, 7];
            case 6:
                res.status(500).json({ message: 'You have to type an URL' });
                _b.label = 7;
            case 7: return [2 /*return*/];
        }
    });
}); });
app.get('/:id', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, originalURL, url, e_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                id = req.params.id;
                return [4 /*yield*/, db.execute({
                        sql: "SELECT original_url FROM url WHERE id = $id",
                        args: { id: id }
                    })];
            case 1:
                originalURL = _a.sent();
                url = originalURL.rows[0];
                res.status(200).json(url);
                return [3 /*break*/, 3];
            case 2:
                e_2 = _a.sent();
                res.status(500).json({ message: 'Something went wrong :,(' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
app.listen(PORT, function () {
    console.log("Server listening on port http://localhost:".concat(PORT));
});
